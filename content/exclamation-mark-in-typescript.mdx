...
The following will return a type `Item | undefined`. In some scenarios, we are not allowed to parse an `undefined` value and therefore leading to further code needed to handle the `undefined` value.
...
Since I have done before Swift Programming or an AR App in Swift - used as the programming language for iOS, iPadOS, macOS, watchOS, the `!` works very similarly and is very commonly used.
